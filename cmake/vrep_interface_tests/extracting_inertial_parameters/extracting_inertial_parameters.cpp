...
This example extracts the inertial parameters of a serial
robot manipulator (FrankaEmikaPanda) from CoppeliaSim.
...
int main(void)
{
    ...
    for (int k=0;k<4;k++)
    {
        ...
        for (int j=0;j<linknames.size();j++)
        {
            ...
            MatrixXd I_absolute_frame = vi.get_inertia_matrix(link, DQ_VrepInterface::ABSOLUTE_FRAME);
